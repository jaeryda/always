...
### 기본 정보
- **서버 호스트**: `192.168.75.85` (또는 `localhost` / `127.0.0.1` - 로컬인 경우)
...
```
서버 호스트: 192.168.75.85 (또는 localhost - 로컬인 경우)
포트: 3306
데이터베이스: always_db
사용자: root
비밀번호: (설치 시 설정한 비밀번호)
...
- 예: `192.168.75.85` (O) / `http://192.168.75.85` (X)
...
**해결 방법:**
1. MySQL 서버(192.168.75.85)에서 MySQL에 접속
