...
**원격 DB 사용 (192.168.75.85 접근 가능한 경우):**
```
set OPENAI_API_KEY=your-openai-api-key-here
set "DATABASE_URL=jdbc:mysql://192.168.75.85:3306/always_db?useSSL=false&serverTimezone=Asia/Seoul&characterEncoding=UTF-8&allowPublicKeyRetrieval=true"
set DATABASE_USERNAME=root
...
- 하지만 `http://192.168.75.85:8089/api/hello` 호출 시 응답 없음
...
1. **네트워크 연결 확인**
   ...
   **Jenkins 서버에서 PowerShell 열기:**
   ...
   ```powershell
   Test-NetConnection -ComputerName 192.168.75.85 -Port 3306
   ```
   ...
   **방법 3: MySQL 클라이언트 사용 (가장 정확)**
   ...
   ```powershell
   mysql -h 192.168.75.85 -u root -p
   ```
   - 연결 성공: MySQL 프롬프트 표시
   - 연결 실패: 오류 메시지 표시
...
1. **프론트엔드 API URL 설정 확인**
   ...
   ```typescript
   export const apiBaseURL = 'http://192.168.75.85:8089/api'
   export const imageBaseURL = 'http://192.168.75.85:8089'
   ```
...
2. **백엔드 서버 실행 확인**
   ...
   또는 브라우저에서:
   ```
   http://192.168.75.85:8089/api/hello
   ```
...
3. **CORS 설정 확인**
   ...
   ```java
   .allowedOrigins("http://localhost:8088", "http://192.168.75.85:8088")
   ```
