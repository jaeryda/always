...
**원격 DB 사용 (192.168.0.2 접근 가능한 경우):**
```
set OPENAI_API_KEY=your-openai-api-key-here
set "DATABASE_URL=jdbc:mysql://192.168.0.2:3306/always_db?useSSL=false&serverTimezone=Asia/Seoul&characterEncoding=UTF-8&allowPublicKeyRetrieval=true"
set DATABASE_USERNAME=root
...
- 하지만 `http://192.168.0.2:8089/api/hello` 호출 시 응답 없음
...
1. **네트워크 연결 확인**
   ...
   **Jenkins 서버에서 PowerShell 열기:**
   ...
   ```powershell
   Test-NetConnection -ComputerName 192.168.0.2 -Port 3306
   ```
...
1. **프론트엔드 API URL 설정 확인**
   ...
   ```typescript
   export const apiBaseURL = 'http://192.168.0.2:8089/api'
   export const imageBaseURL = 'http://192.168.0.2:8089'
   ```
...
2. **백엔드 서버 실행 확인**
   ...
   또는 브라우저에서:
   ```
   http://192.168.0.2:8089/api/hello
   ```
